...
合同会社4U 公式サイト（4u-studio.github.io）の開発ガイドです。

## デザインに関する必須ルール

**デザインを求める意思決定・実装を行う際は、必ず `.claude/skills/` 配下のデザインスキル（awesome-design-skills）を参照すること。**

対象となる作業の例:

- 新しいページ・セクション・コンポーネントの追加やデザイン
- 配色・タイポグラフィ・余白・角丸・影などビジュアルトークンの変更
- レイアウト構成・UIパターンの選定
- ボタン・フォーム・カードなどUI要素のスタイリング
- 既存デザインのリファクタリングやリニューアル

### 参照手順

1. 実装前に、目的に合うデザインスキルを `.claude/skills/<slug>/SKILL.md` で確認する。
2. そのスキルの **Style Foundations / Color / Typography / Spacing / Accessibility / Rules（Do & Don't）** に従って実装する。
3. 設計意図の背景が必要な場合は同フォルダの `DESIGN.md` も参照する。
4. 当サイトの既存トーンとの整合性を保つこと（後述「サイトの既存デザイン傾向」を参照）。

### サイトの既存デザイン傾向

現行の `index.html` は、温かみのある和風エディトリアルなトーン:

- 背景: クリーム / 和紙色（`--cream:#f6f0e2`, `--paper:#fffdf6`）
- アクセント: 深緑・黄土色（ochre）・テラコッタ・林檎赤
- フォント: 見出しに `Shippori Mincho`（明朝）、本文に `Zen Kaku Gothic New`
- 角丸・柔らかい影・微細なノイズテクスチャ

この方向性に最も近いスキルは `terracotta` / `editorial` / `paper` / `refined` / `elegant`。
新規デザイン時はまずこれらを起点に検討する。

## インストール済みデザインスキル一覧（awesome-design-skills）

各スキルは `.claude/skills/<slug>/` に `SKILL.md`（AI向け指示）と `DESIGN.md`（設計ドキュメント）を含む。
出典: https://github.com/bergside/awesome-design-skills
...
- `claude` — A research-journal aesthetic printed on warm stone — authoritative, editorial, almost achromatic.
...
- `codex` — A radically minimal, blank-canvas interface built as a pure edge-to-edge surface, with almost no color and typography carrying the visual weight.
...
- `fiction` — A playful, energetic, cartoonesque interface inspired by friendly children's-book illustrations.
...
- `immersive` — An immersive, interactive, exhibit-style interface that blends storytelling, animation, and gamified elements.
- `impeccable` — A modern, graphic, editorial-poster aesthetic — warm and confident — built on alternating cream and burnt orange sections.
- `levels` — Conversion-focused design that removes friction and guides users toward action through clarity, trust, and speed.
- `lingo` — Playful, minimal design with bright colors, rounded shapes, tactile 3D borders, and friendly illustrations.
...
- `matrix` — A cyber-slick, dark-only Matrix-inspired interface defined by minimalist fashion and high-tech digital elements.
...
- `riso` — A playful, joyful, two-color risograph print aesthetic built on a single warm off-white paper surface.
- `sega` — A playful, arcade-inspired interface for games — built on the VT323 pixel typeface, hard-edged corners, and chunky pill buttons.
...
- `sketch` — A friendly, hand-drawn sketch interface inspired by pencil illustrations on warm cream paper.
...
TypeUI CLI で追加・更新できる:

```bash
npx typeui.sh list           # 利用可能なスキル一覧
npx typeui.sh pull <slug>    # 指定スキルを取得
```
